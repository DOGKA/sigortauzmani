-- Otomatik üretildi: npm run legal:seed (scripts/generate-legal-seed-sql.mjs)
-- Kaynak: src/data/legal.ts — sitede yayında olan TR metinler.
...
values ($q0$kvkk$q0$, 'tr', $q0$Kişisel Verilerin İşlenmesine İlişkin Aydınlatma Metni$q0$, $q0$6698 sayılı Kişisel Verilerin Korunması Kanunu kapsamında kişisel verilerin işlenmesine ilişkin aydınlatma metni. Veri sorumlusu, işlenen veriler, amaçlar, aktarım ve haklarınız.$q0$)
...
select
  ...
  "summary": "KVKK aydınlatma metni: veri sorumlusu, işlenen veriler, amaçlar, aktarım, otomatik işlemler ve başvuru hakları.",
  "intro": [],
  "sections": [
    {
      "heading": "1. Veri sorumlusu",
      "paragraphs": [
        "6698 sayılı Kişisel Verilerin Korunması Kanunu (“KVKK”) kapsamında kişisel verileriniz, veri sorumlusu sıfatıyla GROSS SİGORTA ARACILIK HİZMETLERİ LİMİTED ŞİRKETİ (“Şirket”) tarafından işlenmektedir.",
        "KEP: grosssigorta@hs03.kep.tr",
        "KVKK e-posta adresi: sigorta@sigortauzmani.net",
        "Telefon: 0850 302 00 32",
        "TOBB Sigorta Acenteleri Levha No: G08612-15EG",
        "MERSİS No: 0411085688900001",
        "Ticaret Sicil / Dosya No: 6422-5",
        "Vergi Kimlik No: 4110856889",
        "Vergi Dairesi: Kartal"
      ]
    },
    {
    ...
    },
    {
      ...
      "paragraphs": [
        "Kişisel verileriniz, işleme faaliyetine göre;"
      ],
      "items": [
    ...
    },
    {
      ...
      "closing": [
        ...
        "KEP: grosssigorta@hs03.kep.tr",
        "Kayıtlı e-posta adresiniz üzerinden: sigorta@sigortauzmani.net",
        "Güvenli elektronik imza veya mobil imza ile imzaladığınız başvurular: sigorta@sigortauzmani.net",
...
-- Gizlilik Politikası
insert into public.legal_documents (slug, locale, title, description)
values ($q0$gizlilik-politikasi$q0$, 'tr', $q0$Gizlilik Politikası$q0$, $q0$sigortauzmani.net gizlilik politikası. Teklif süreci, form ve dosyalar, ödeme güvenliği, hizmet mesajları ve bilgi güvenliği.$q0$)
on conflict (slug, locale) do update
set title = excluded.title,
    description = excluded.description;
...
from public.legal_documents d
where v.document_id = d.id
  and d.slug = $q0$gizlilik-politikasi$q0$
  and d.locale = 'tr'
  and v.status = 'published'
  and v.version <> 1;
...
select
  ...
  "updatedAt": "1 Eylül 2026",
  "summary": "Gizlilik politikası: teklif süreci, form ve dosyalar, ödeme güvenliği, hizmet mesajları ve bilgi güvenliği.",
  "intro": [
  ...
  '2026-09-01T00:00:00+03:00'::timestamptz
from public.legal_documents d
where d.slug = $q0$gizlilik-politikasi$q0$
  and d.locale = 'tr'
on conflict (document_id, version) do update
set status = 'published',
    content = excluded.content,
...
insert into public.legal_documents (slug, locale, title, description)
values ($q0$cerez-politikasi$q0$, 'tr', $q0$Çerez ve Benzeri Teknolojiler Politikası$q0$, $q0$sigortauzmani.net çerez ve benzeri teknolojiler politikası. Google analiz araçları, kullanılan veriler ve tercih yönetimi.$q0$)
on conflict (slug, locale) do update
set title = excluded.title,
    description = excluded.description;
...
from public.legal_documents d
where v.document_id = d.id
  and d.slug = $q0$cerez-politikasi$q0$
  and d.locale = 'tr'
  and v.status = 'published'
  and v.version <> 1;
...
select
  ...
  "summary": "Çerez ve benzeri teknolojiler politikası: Google analiz araçları, kullanılan veriler, yurt dışı aktarım ve tercih yönetimi.",
  "intro": [],
  "sections": [
    {
    ...
    },
    {
      "heading": "6. İletişim",
      "paragraphs": [
        "Çerez ve benzeri teknolojilerle ilgili sorularınızı sigorta@sigortauzmani.net adresine iletebilirsiniz."
      ]
    }
  ]
}$q0$::jsonb,
  ...
  '2026-09-11T00:00:00+03:00'::timestamptz
from public.legal_documents d
where d.slug = $q0$cerez-politikasi$q0$
  and d.locale = 'tr'
on conflict (document_id, version) do update
set status = 'published',
    content = excluded.content,
...
-- KVKK Başvuru Formu
insert into public.legal_documents (slug, locale, title, description)
values ($q0$kvkk-basvuru$q0$, 'tr', $q0$KVKK Başvuru Formu$q0$, $q0$6698 sayılı KVKK kapsamında kişisel veri taleplerinizi iletebileceğiniz başvuru kanalları, gerekli bilgiler, talep konuları ve sonuçlandırma süresi.$q0$)
on conflict (slug, locale) do update
set title = excluded.title,
    description = excluded.description;
...
select
  ...
  "eyebrow": "Kişisel verilerin korunması",
  "updatedAt": "1 Eylül 2026",
  "summary": "KVKK başvuru kanalları, başvuruda yer alması gereken bilgiler, talep konuları ve sonuçlandırma süresi.",
  "intro": [
    "6698 sayılı Kişisel Verilerin Korunması Kanunu’nun 11. maddesi kapsamındaki taleplerinizi GROSS SİGORTA ARACILIK HİZMETLERİ LİMİTED ŞİRKETİ’na iletebilirsiniz."
  ],
  "sections": [
    {
      "heading": "Başvuru kanalları",
      "paragraphs": [
        "KEP adresi: grosssigorta@hs03.kep.tr",
        "Şirket sisteminde kayıtlı e-posta adresiniz üzerinden: sigorta@sigortauzmani.net",
        "Güvenli elektronik imza veya mobil imza ile imzaladığınız başvurular: sigorta@sigortauzmani.net"
      ]
    },
    {
      ...
      "paragraphs": [
        "Başvurunuzda aşağıdaki bilgilere yer verin:"
      ],
      "items": [
        "Ad ve soyad",
  ...
  '2026-09-01T00:00:00+03:00'::timestamptz
from public.legal_documents d
where d.slug = $q0$kvkk-basvuru$q0$
  and d.locale = 'tr'
on conflict (document_id, version) do update
set status = 'published',
    content = excluded.content,
