 ...
 *
 * Sonuç yalnızca bilgilendirme içindir: kayıt varsa ziyaretçiye "daha önce
 * teklif çalışılmış, güncel fiyatlar alınacak" denir; teklif akışı her
 * durumda yeni teklif açar.
 *
...
import { kayitliTeklifBul } from "../_shared/uye";
...
export default async function handler(request: Request): Promise<Response> {
  ...
  // Plaka + ruhsat tamamlanınca kişi başına bir kez tetikleniyor; tavan teklif
  // ucundaki IP limitiyle (200/saat) aynı.
  const allowed = await rateCheck(ipHash, "kayitli_teklif", 200, 3600);
  if (!allowed) {
    return withCookie(
  ...
  const teklif = await kayitliTeklifBul(plaka, kimlikNo, bransNo);
