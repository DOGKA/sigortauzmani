...
import { requireUser, serviceClient } from "@/lib/settings/server";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Belge oluşturulamadı." },
      { status: 400 },
    );
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Sürüm güncellenemedi." },
      { status: 400 },
    );
  }
}
