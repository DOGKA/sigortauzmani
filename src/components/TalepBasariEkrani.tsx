...
interface Props {
  talepNo: string;
  /** Başlıkta ve WhatsApp mesajında geçen ürün adı. */
  urunAdi: string;
  /** WhatsApp mesajına eklenen satır (ör. seçilen sigorta şirketi). */
  whatsappEkSatiri?: string;
  onYeniTeklif: () => void;
...
export default function TalepBasariEkrani({
  ...
  const whatsappMessage = [
    `Merhaba, sigortauzmani.net üzerinden ${urunAdi} teklif talebi oluşturdum.`,
    talepNo ? `Talep numaram: ${talepNo}` : null,
    whatsappEkSatiri ?? null,
    ...
    <div className="quote__success">
      ...
      <div className="quote__success-hero">
        ...
        <h2 className="quote__success-title">
          {urunAdi} teklif talebiniz alındı
        </h2>
        <p className="quote__success-sub">
          Uzmanımız en kısa sürede sizinle iletişime geçecek. Dilerseniz
          aşağıdan iletişim tercihinizi belirleyin.
        </p>
      </div>
      ...
      <div className="quote__success-grid">
        ...
        <section className="quote__panel quote__panel--wa">
          ...
          <p className="quote__panel-note">
            +90 850 302 00 32 numaralı hattımıza yönlendirileceksiniz. Mesajınız
            hazır olacak, yalnızca göndermeniz yeterli.
          </p>
        </section>
      </div>
