...
/** Paylaşım kartları /api/og tarafından üretilir; parametresiz çağrı site kartını verir. */
...
/** Blog yazısı paylaşım kartı (1200×630 PNG). */
...
/** Blog dışı sayfalar için başlığı karta basılan paylaşım görseli. */
export function pageOgImageUrl(title: string, label?: string): string {
  const params = new URLSearchParams({ title: title.slice(0, 120) });
  if (label) params.set("label", label.slice(0, 40));
